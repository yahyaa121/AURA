...
<head>
  ...
  <title>Special Offers - Perfume Shop</title>
  <style>
    * {
      margin: 0;
      padding: 0;
...
<body>
 ...
 <?php
// Récupérer les 6 produits en promotion (sans filtre de date)
$query = "
SELECT p.idPerfume, p.perfumeName, p.price, c.collectionName, pi.urlImage, pi.urlHover,
       d.reduction
FROM discounts d
JOIN perfumes p ON d.idPerfume = p.idPerfume
JOIN collections c ON p.idCollection = c.idCollection
JOIN perfumeimage pi ON p.idPerfume = pi.idPerfume
ORDER BY d.idDiscount ASC
LIMIT 6
";
...
?>
  ...
  <div class="main-content">
    <div class="section-title">
      <h2 style="font-size: 24px;"><?= $lang["offers"] ?></h2>
    </div>
    ...
    <div class="products">
     <?php foreach($products as $product): ?>
      <div class="product">
        ...
        <div class="product-brand"><?= htmlspecialchars($product['collectionName']) ?></div>
        <div class="product-name"><?= htmlspecialchars($product['perfumeName']) ?></div>
        <div class="product-price">
          $<?= number_format($product['price'], 2) ?>
          <span style="color:#bfa46f;font-size:13px;font-weight:bold;margin-left:8px;">
            -<?= rtrim(rtrim($product['reduction'], '0'), '.') ?>%
          </span>
        </div>
      </div>
    <?php endforeach; ?>
